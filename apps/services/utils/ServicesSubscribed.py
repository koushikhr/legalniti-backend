...
@csrf_exempt
def ServicesSubscribed(request):
        ...
        if auth_header and auth_header.startswith('Bearer '):
            ...
            if user:
                ...
                for relation in relations:
                    ...
                    servicesList[relation["_id"]] = relation["services"]
                if (servicesList):
                    return JsonResponse({"ServicesSubscribed": servicesList}, status=200)

        return JsonResponse({"Authentication": "Error"}, status=401)
